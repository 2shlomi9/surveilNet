...
class Absent:
    def __init__(self, id, first_name, last_name, img_path=None, age=None, embedding=None):
        self.id = id
        self.first_name = first_name
        self.last_name = last_name
        self.img_path = img_path
        self.age = age
        # If embedding is provided (from Firebase), store it as np.array
        if embedding is not None:
            # support both single vector or list of vectors
            if isinstance(embedding[0], list) or isinstance(embedding[0], np.ndarray):
                self._embedding = [np.array(e, dtype=np.float32) for e in embedding]
            else:
    ...
    def get_emb(self):
        """Compute embedding(s) once from img_path if not provided"""
        if self._embedding is None and self.img_path:
            if not os.path.exists(self.img_path):
                print(f"[ERROR] Image not found: {self.img_path}")
                return None
            ...
            self._embedding = [emb]  # store as list
        return self._embedding
    ...
    def to_dict(self):
        """Convert to dictionary for Firestore or JSON"""
        embeddings = [e.tolist() for e in self.get_emb()] if self.get_emb() else None
        return {
            "id": self.id,
            "first_name": self.first_name,
            "last_name": self.last_name,
